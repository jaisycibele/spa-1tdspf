        ...
        <tbody> 
          {ListaProdutos.map((produto, indice) => (
            <tr key={indice} className={estilos.tblLine}>
              ...
              <td>{produto.desc}</td>
              <td>{produto.preco}</td>
              <td><button onClick={handleStatus(true,produto.id)}>Abrir Modal</button> <Link to={`/editar/produtos/${produto.id}`}><EditObj/></Link> | <Link to={`/excluir/produtos/${produto.id}`}><DelObj/></Link></td>
            </tr>
          ))}
        </tbody>
        <tfoot>
